...
void print_irq_info(int i)
{
	...
	if (zte_irq_desc) {
		...
		/*
		pr_info("[IRQ] IRQ-NUM=%d\n", i);
		pr_info("		irq chip->name=%10s\n", zte_irq_desc->irq_data.chip->name ? : "-");
		pr_info("		irq action->name=%s\n", action->name);
		if (wake_prop > 0)
			pr_info("		irq wake_depth=%d\n", wake_prop);
		*/

		/*special state:if (!strcmp(action->name, "qcom,smd-modem"))*/

		for (action = action->next; action; action = action->next)
			pr_info("		show all action->name=%s\n", action->name);
