			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] != 0) {
		for (i = 0; i < tfa98xx_cnt_max_device(); i++) {
			...
			if (tfa98xx) {
				pr_info("tfa98xx_set_stop_ctl: handle %d, ctl %ld\n",
						tfa98xx->handle, ucontrol->value.integer.value[0]);
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (!tfa98xx->input) {
		...
		if (!(val & TFA98XX_STATUSREG_CLKS) ||
			...
			pr_err("handle(%d) Misc errors detected: STATUS_FLAG0 = 0x%x\n", val, tfa98xx->handle);
		...
		if (tfa98xx_dev_family(tfa98xx->handle) == 2) {
			...
			if ((val & TFA98XX_STATUS_FLAGS1_TDMERR) |
				...
				pr_err("handle(%d) TDM related errors: STATUS_FLAG1 = 0x%x\n", val, tfa98xx->handle);
		}
	}
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	dev_info(&tfa98xx->i2c->dev, "state: %d\n", mute);
