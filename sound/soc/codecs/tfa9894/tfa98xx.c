...
/* ZTE_chenjun: Start */
#include <soc/qcom/vendor/board_id.h>
/* ZTE_chenjun: End */
...
/* ZTE_chenjun: Start */
static char *fw_name_default;
static char *fw_name_older = "tfa98xx_older.cnt";

static const int older_board_id[] = { 8, 10, 11 };
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	for (i = 0; i < ARRAY_SIZE(older_board_id); i++) {
		if (older_board_id[i] == board_id) {
			fw_name_default = fw_name;
			fw_name = fw_name_older;
			break;
		}
	}

	pr_info("board_id %d, fw_name %s\n", board_id, fw_name);
