...
/* kernel write file start*/
#include <linux/fs.h>
#include <asm/uaccess.h>
#include <linux/err.h>
#include <linux/workqueue.h>
/* kernel write file end*/

/*#define ENABLE_KERNEL_WR_FILE 1 */
#ifdef ENABLE_KERNEL_WR_FILE
#define MAX_EEPROM_DATA_LEN  10240
struct delayed_work my_delay_work;
struct workqueue_struct *my_work_queue;
uint8_t *data_buf = NULL;
uint32_t eeprom_len;
uint8_t *save_eeprom_filename;
uint8_t save_eeprom_filename_path[200] = {"/data/"};
#endif
...
#ifdef ENABLE_KERNEL_WR_FILE
void kernel_save_file(uint8_t *file_name, uint8_t *src_buf, uint32_t len)
{
	...
	if (IS_ERR(file)) {
		CAM_ERR(CAM_EEPROM, "Error occurred while opening file %s\n", file_name);
	} else {
		old_fs = get_fs();
		set_fs(KERNEL_DS);
		pos = 0;
		vfs_write(file, src_buf, len, &pos);
		pos = 0;
		filp_close(file, NULL);
		set_fs(old_fs);
		file = NULL;
		CAM_ERR(CAM_EEPROM, "save file %s ok\n", file_name);
	}
	kfree(data_buf);
	data_buf = NULL;
}
void save_eeprom_data_to_file(struct work_struct *work)
{
	strlcat(save_eeprom_filename_path, save_eeprom_filename, sizeof(save_eeprom_filename_path));
	kernel_save_file(save_eeprom_filename_path, data_buf, eeprom_len);
}
#endif
...
#ifdef ENABLE_KERNEL_WR_FILE
	else {/*opt data is ok, save it to file */
		if (e_ctrl->eeprom_fun_p->get_default_eeprom_setting_filename) {
			save_eeprom_filename = e_ctrl->eeprom_fun_p->get_default_eeprom_setting_filename(e_ctrl);
			data_buf = kzalloc(MAX_EEPROM_DATA_LEN, GFP_KERNEL);
			if (!data_buf) {
				CAM_ERR(CAM_EEPROM, "malloc fail\n");
			} else {
				my_work_queue = create_workqueue("my workqueue");
				if (!my_work_queue) {
					CAM_ERR(CAM_EEPROM, "Create workqueue failed!\n");
				} else {
					INIT_DELAYED_WORK(&my_delay_work, save_eeprom_data_to_file);
					eeprom_len = e_ctrl->cal_data.num_data;
					memcpy(data_buf, &(e_ctrl->cal_data.mapdata[0]), eeprom_len);
					queue_delayed_work(my_work_queue, &my_delay_work, msecs_to_jiffies(30000));
				}
			}
		}
	}
...
static int32_t cam_eeprom_pkt_parse(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	...
	case CAM_EEPROM_PACKET_OPCODE_INIT:
		...
		CAM_ERR(CAM_EEPROM, "e_ctrl->userspace_probe=%d", e_ctrl->userspace_probe);
...
int32_t zte_cam_eeprom_driver_cmd(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	...
	case CAM_RELEASE_DEV:
		CAM_ERR(CAM_EEPROM, "CAM_RELEASE_DEV");
		if (e_ctrl->cam_eeprom_state != CAM_EEPROM_ACQUIRE) {
			rc = -EINVAL;
			CAM_WARN(CAM_EEPROM,
