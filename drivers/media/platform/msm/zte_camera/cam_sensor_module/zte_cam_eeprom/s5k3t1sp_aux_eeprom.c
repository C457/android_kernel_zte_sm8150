...
#define S5K3T1SP_AUX_LENS_ID_ADDR 0x2
...
void s5k3t1sp_write_fix_info_eeprom(struct cam_eeprom_ctrl_t *e_ctrl)
{
	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR] = 0x04;
	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 1] = 0x30;

	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 2] = 0x04;
	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 3] = 0x00;

	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 4] = 0x12;
	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 5] = 0x93;

	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 6] = 0x04;
	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 7] = 0x00;

	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 8] = 0x1e;
	e_ctrl->cal_data.mapdata[DUAL_AEC_S_ADDR + 9] = 0x9d;
}
...
int s5k3t1sp_aux_checksum_eeprom(struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->cal_data.num_data >= (DUAL_AEC_S_ADDR + DUAL_AEC_LEN))
		s5k3t1sp_write_fix_info_eeprom(e_ctrl);
...
uint32_t s5k3t1_aux_read_lens_id(struct cam_sensor_cci_client *cci_client)
{
	int rc = 0;
	struct cam_sensor_cci_client local_cci_client;
	uint32_t reg_addr = S5K3T1SP_AUX_LENS_ID_ADDR;
	uint32_t data = 0;
	uint32_t lens_id = 0;
...
static struct zte_eeprom_fn_t s5k3t1sp_aux_eeprom_func_tbl = {
	...
	.read_id = s5k3t1_aux_read_lens_id,
