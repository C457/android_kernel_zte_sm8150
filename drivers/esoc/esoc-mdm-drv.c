...
/* initial value of continuously trying instead of panic */
/* And it is no matter with a value more than the max of app can set below */
#define ESOC_DEF_PON_REQ	6
/* do cold reset once instead of warm reset every 3 retries */
#define ZTE_ACTION_COLD_RATIO 3
#else
...
#ifdef CONFIG_ESOC_PON_FAIL_ZTE /* zte_5g_pon add */
/* how many is powerup called totally */
static unsigned int zte_esoc_powerup_count = 0;
/* how many is pon_state success totally */
static unsigned int zte_esoc_power_succ_count = 0;
/* the max panic count ever reached during every powerup */
static unsigned int zte_esoc_max_count = 0;
/* the last powerup panic count, reset before each powerup */
...
/* check whether do a cold reset instead of warm one */
...
/* to deal very little boot stuck and timeout issue, here */
/* forbid a powerdown and retry right after */
...
#ifdef CONFIG_ESOC_PON_FAIL_ZTE /* zte_5g_pon add */
		/* for dead loop with retry, long timeout set */
		/* so if a device with wrong 5g, close by hand */
		zte_esoc_add_panic_cnt(1); /* increment 1 */
		if (zte_esoc_check_cold_action()) {
			/* do panic as S3 RESET except pon trial cnt */
			mdm_subsys_retry_powerup_cleanup(esoc_clink,
							ESOC_HOOK_MDM_DOWN);
			esoc_mdm_log("Cold S3 reset for panic\n");
			mdm_power_down(mdm);
			msleep(S3_RESET_DELAY_MS);
		}
#else
...
static int mdm_subsys_powerup(const struct subsys_desc *crashed_subsys)
{
	...
	do {
		...
		if (esoc_clink->auto_boot)
			timeout = 10 * HZ;
#ifdef CONFIG_ESOC_PON_FAIL_ZTE /* zte_5g_pon add */
		/* override to save power with dead loop */
		timeout = 120 * HZ;
#endif
