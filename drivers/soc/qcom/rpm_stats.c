...
void pm_show_rpm_stats(void)
{
	...
	pdata = rpm_data;
	if (!pdata) {
		pr_err("%s: ERROR msm_rpmstats_platform_data=NULL\n", __func__);
		return;
	}
	...
	if (sscanf(temp+strlen("cxsd\n\t count:"), "%llu\n", &count) == 1) {
		if (vmin_count != count) {
			pr_info("count: last %llu now %llu , enter cxsd success\n", vmin_count, count);
			vmin_count = count;
			debug_suspend_disable();
		} else {
			pr_info("count: last %llu now %llu, enter cxsd failed\n", vmin_count, count);
