...
/*zte_pm*/
static ssize_t pm_show_print_rpmh(char *prvbuf, ssize_t length,
				struct msm_rpmh_master_stats *record,
				const char *name)
{
	uint64_t accumulated_duration = record->accumulated_duration;
	...
	return snprintf(prvbuf, length, "%s, Version:0x%x,Sleep Count:0x%x,"
...
void pm_show_rpmh_master_stats(void)
{
	...
	length = pm_show_print_rpmh(buf, RPMH_SHOW_SIZE, &apss_master_stats, "APSS");
	pr_info("rpmh::%s\n", buf);
	...
	for (i = 0; i < ARRAY_SIZE(rpmh_masters); i++) {
		record = (struct msm_rpmh_master_stats *) qcom_smem_get(
			...
			if (!IS_ERR_OR_NULL(record) && (RPMH_SHOW_SIZE - length > 0)) {
				length += pm_show_print_rpmh(
					buf, RPMH_SHOW_SIZE, record, rpmh_masters[i].master_name);
				pr_info("rpmh::%s\n", buf);
		   } else {
