...
static const unsigned char table_8[256] = {
...
unsigned char get_crc8_rohc(unsigned char start, const unsigned char *buff, int len)
{
	...
	for (i = 0; i < len; i++) {
		accu = table_8[accu ^ buff[i]];
	}
	return accu;
}


/*unsigned short crc16_ccitt_false(unsigned short start, unsigned short *data, unsigned int length)
{
	unsigned char i;
	unsigned short crc = start;//Initial value
	while (length--) {
		crc ^= (unsigned short)(*data++); //crc ^= (uint16_t)(*data)<<8; data++;
		for (i = 0; i < 16; ++i) {
			if (crc & 0x8000)
				crc = (crc << 1) ^ 0x1021;
			else
...
{
	unsigned short accu = start;
	unsigned int i = 0;
	unsigned char even = len & 0x01;

	/*little endian*/
	for (i = 0; i < len / 2; i++) {
		accu = (accu << 8) ^ table16[(accu >> 8) ^ buff[2 * i + 1]];
		accu = (accu << 8) ^ table16[(accu >> 8) ^ buff[2 * i]];
	}
	if (even) {
		accu = (accu << 8) ^ table16[(accu >> 8) ^ 0x00];
		accu = (accu << 8) ^ table16[(accu >> 8) ^ buff[len - 1]];
	}
