...
uint8_t *himax_flash_tmp_buffer_alloc;
uint8_t *himax_report_buffer_alloc;
...
int himax_gpio_power_config(struct himax_i2c_platform_data *pdata)
{
	...
	error = regulator_enable(pdata->vcc_dig);
	if (error) {
		E("%s: Failed to enable regulator vdd\n",
			 __func__);
	}
							...
							unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == MSM_DRM_EVENT_BLANK && ts &&
	    ts->client) {
		blank = evdata->data;
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	himax_flash_tmp_buffer_alloc = kzalloc(sizeof(uint8_t) * 200, GFP_KERNEL);
	if (himax_flash_tmp_buffer_alloc == NULL) {
		E("%s: allocate himax_flash_tmp_buffer_alloc failed!\n", __func__);
		ret = -ENOMEM;
		goto err_alloc_flash_tmp_buffer;
	}

	himax_report_buffer_alloc = kzalloc(sizeof(uint8_t) * 128, GFP_KERNEL);
	if (himax_report_buffer_alloc == NULL) {
		E("%s: himax_report_buffer_alloc failed!\n", __func__);
		ret = -ENOMEM;
		goto err_himax_report_buffer;
	}
...
err_himax_report_buffer:
	kfree(himax_flash_tmp_buffer_alloc);
err_alloc_flash_tmp_buffer:
	kfree(ts);
...
int himax_chip_common_remove(struct i2c_client *client)
{
	...
	kfree(himax_flash_tmp_buffer_alloc);
	kfree(himax_report_buffer_alloc);
	return 0;
}
