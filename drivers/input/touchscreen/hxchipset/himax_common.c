...
extern uint8_t *himax_report_buffer_alloc;
...
mem_alloc_eventbuf_fail:
	kfree(hx_touch_data->hx_rawdata_buf);
#endif
mem_alloc_rawdata_fail:
	kfree(g_target_report_data->finger_id);
mem_alloc_fingeriddata_fail:
	kfree(g_target_report_data->w);
mem_alloc_wdata_fail:
	kfree(g_target_report_data->y);
mem_alloc_ydata_fail:
	kfree(g_target_report_data->x);
mem_alloc_xdata_fail:
	kfree(g_target_report_data);
	g_target_report_data = NULL;
mem_alloc_reportdata_fail:
	kfree(hx_touch_data->hx_coord_buf);
mem_alloc_hxcoord_fail:
...
{
	if (g_ts_dbg != 0)
		I("%s: Entering, ts_status=%d! ts_path=%d\n", __func__, ts_status, ts_path);
...
static int himax_ts_operation(struct himax_ts_data *ts, int ts_path, int ts_status)
{
	...
	uint8_t *buf = himax_report_buffer_alloc;
