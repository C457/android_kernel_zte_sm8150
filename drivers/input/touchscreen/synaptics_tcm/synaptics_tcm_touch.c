...
static void touch_free_objects(void)
{
	...
	if (area_meet_state) {
		...
		area_meet_state = 0;
		input_report_key(touch_hcd->input_dev, KEY_AREAMEET_UP, 1);
		input_sync(touch_hcd->input_dev);
		input_report_key(touch_hcd->input_dev, KEY_AREAMEET_UP, 0);
		input_sync(touch_hcd->input_dev);
	}
...
static inline void report_uevent(struct syna_tcm_hcd *tcm_hcd, char **envp)
{
...
static void touch_report(void)
{
	...
	if (touch_data->fingerprint_area_meet && !area_meet_state) {
		if (tcm_hcd->in_suspend) {
			...
			area_meet_state = 1;
			report_uevent(tcm_hcd, envp);
			input_report_key(touch_hcd->input_dev, KEY_PRESSURE, 1);
			input_sync(touch_hcd->input_dev);
			input_report_key(touch_hcd->input_dev, KEY_PRESSURE, 0);
			input_sync(touch_hcd->input_dev);
		} else {
...
/*	set_bit(KEY_AOD_AREAMEET_DOWN, touch_hcd->input_dev->keybit);*/
	...
	set_bit(KEY_PRESSURE, touch_hcd->input_dev->keybit);
	set_bit(KEY_AREAMEET_UP, touch_hcd->input_dev->keybit);
