...
static ssize_t
gt_proc_read_val(struct file *file, char __user *page, size_t size, loff_t *ppos)

{
	...
	if (ret >= 0) {
		if (ic_version[5] == 0x00) {
			len += snprintf(buffer + len, sizeof(buffer) - len, "chip type : %c%c%c\n",
				ic_version[2], ic_version[3], ic_version[4]);
		} else {
			if (ic_version[5] == 'S' || ic_version[5] == 's') {
				len += snprintf(buffer + len, sizeof(buffer) - len, "chip type : %c%c%c%c\n",
					ic_version[2], ic_version[3], ic_version[4], ic_version[5]);
			}
		}
	}
#endif
	len += snprintf(buffer + len, sizeof(buffer) - len, "sensor id : %d\n", gtp_ts->sensor_id);
	len += snprintf(buffer + len, sizeof(buffer) - len, "fw version : %04x\n",
				(ic_version[7] << 8) | ic_version[6]);
	len += snprintf(buffer + len, sizeof(buffer) - len, "update flag : 0x%x\n", goodix_update_flag);

	if (strcmp(gtp_file_name, "")) {
	len += snprintf(buffer + len, sizeof(buffer) - len, "fw file name : %s\n", gtp_file_name);

	GTP_INFO("etc path:%s\n", gtp_etc_path);
		if (get_gtp_mounle_fw_info(&fw_head) >= 0) {
			len += snprintf(buffer + len, sizeof(buffer) - len, "fw file info  : %d%d%d%d\n",
				fw_head.hw_info[0], fw_head.hw_info[1],
				fw_head.hw_info[2], fw_head.hw_info[3]);
			len += snprintf(buffer + len, sizeof(buffer) - len, "fw file pid : %s\n", fw_head.pid);
			len += snprintf(buffer + len, sizeof(buffer) - len, "fw file vid : %4x\n",
				((fw_head.vid&0xff)<<8)|((fw_head.vid&0xff00)>>8));
			if ((((fw_head.vid&0xff)<<8)|((fw_head.vid&0xff00)>>8))
							> ((ic_version[7] << 8) | ic_version[6]))
				len += snprintf(buffer + len, sizeof(buffer) - len, "you should update the ts fw\n");

		} else
			len += snprintf(buffer + len, sizeof(buffer) - len, "can not update fw now\n");
	} else
		len += snprintf(buffer + len, sizeof(buffer) - len, "there is no match fw to update\n");
...
static ssize_t gt_proc_write_val(struct file *file, const char  __user *buffer,
...
static const struct file_operations proc_ops = {
	.owner = THIS_MODULE,
	.read = gt_proc_read_val,
	.write = gt_proc_write_val,
};
...
/*	ret = gtp_init_panel(ts);*/
	...
	goodix_irq_ts_work_buf = kzalloc(8 * GTP_MAX_TOUCH * sizeof(u8), GFP_KERNEL);
	if (goodix_irq_ts_work_buf == NULL) {
		GTP_ERROR("request mem for goodix_irq_ts_work_buf is fail!");
		ret = -ENOMEM;
		goto err_goodix_irq_ts_work_buf;
	}
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	if (!ts) {
		GTP_ERROR("%s: goodix_ts_data is null!\n", __func__);
		return -EINVAL;
	}
