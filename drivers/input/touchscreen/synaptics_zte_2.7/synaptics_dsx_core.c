...
#define FW_IMAGE_NAME_SAMSUNG      "synaptics/startup_fw_update_samsung.img"
#define FW_IMAGE_NAME_GVO      "synaptics/startup_fw_update_gvo.img"
...
static struct workqueue_struct *syna_rmi4_probe_wq;
static struct work_struct	syna_rmi4_probe_work;

unsigned char user_value[USER_BUF_SIZE * 2] = {0};
...
extern unsigned char *syna_file_name;
...
extern int zte_write_lockdown_info(void);
...
extern int syna_get_fw_ver(struct i2c_client *client, unsigned char *pfwfile);
...
extern int syna_get_customer_id(struct rmi_config_id *config_id);
...
#endif
	/*add by wkh*/
	__ATTR(suspend, (/*S_IWUSR | S_IWGRP*/0220),
			synaptics_rmi4_show_error,
	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (!rmi4_data->suspend)
		synaptics_rmi4_glove_mode_enable(rmi4_data, rmi4_data->enable_glove);
	else
		pr_notice("%s: suspend mode, do not write reg!\n", __func__);
	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	dev_info(rmi4_data->pdev->dev.parent,
		"%s: enable_smart_cover = %d, suspend = %d\n",
		__func__,
		rmi4_data->enable_smart_cover,
		rmi4_data->suspend);

	if (!rmi4_data->suspend)
		synaptics_rmi4_smart_cover_enable(rmi4_data, rmi4_data->enable_smart_cover);
	else
		pr_notice("%s: suspend mode, do not write reg!\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input == 1)
		...
		schedule_work(&syna_rmi4_suspend_work);
	else if (input == 0)
		/*synaptics_rmi4_resume(dev);*/
		schedule_work(&syna_rmi4_resume_work);
	else
		return -EINVAL;
...
#define ZTE_CIRCLE_CENTER_Y 1741
#define ZTE_CIRCLE_RADIUS 134
#define KEY_PRESSURE 0x2a8
#define PRESSURE_CALIBRATION 97
#define PRESSURE_THRESHOLD 28

static char force_touch_id = -1;
static unsigned char report_key_flag = 0;

int zte_in_circle(int x, int y)
{
	int ret = 0;
	int distance_x = abs(x - ZTE_CIRCLE_CENTER_X);
	int distance_y = abs(y - ZTE_CIRCLE_CENTER_Y);
	int distance_z = int_sqrt(distance_x * distance_x + distance_y * distance_y);

	if (distance_z > ZTE_CIRCLE_RADIUS)
		ret = -1;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	int retval;
	unsigned char touch_count = 0; /* number of touch points */
	static unsigned char max_touch_num = 0;
	unsigned char index;
	unsigned char finger;
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if ((!finger_presence) && (finger == 0))
			pr_notice("%s : touch down[%u,%u]\n", __func__, x, y);
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (rmi4_data->report_pressure) {
				...
				pressure = pressure / PRESSURE_CALIBRATION;
				input_report_abs(rmi4_data->input_dev,
						ABS_MT_PRESSURE, pressure);
				pr_info("%s: pressure_report=%d\n", __func__, pressure);
#ifdef ZTE_REPORT_KEY_PRESSURE
				retval = zte_in_circle(x, y);
				if (!retval && (pressure >= PRESSURE_THRESHOLD) && !report_key_flag) {
					pr_info("%s: report KEY_PRESSURE down\n", __func__);
					input_report_key(rmi4_data->input_dev, KEY_PRESSURE, 1);
					input_sync(rmi4_data->input_dev);
					report_key_flag = 1;
					force_touch_id = finger;
				} else {
					if (report_key_flag && (force_touch_id == finger) &&
						((pressure < PRESSURE_THRESHOLD) || retval)) {
						pr_info("%s: report KEY_PRESSURE up\n", __func__);
						input_report_key(rmi4_data->input_dev, KEY_PRESSURE, 0);
						input_sync(rmi4_data->input_dev);
						report_key_flag = 0;
						force_touch_id = -1;
					}
	...
	max_touch_num = (touch_count > max_touch_num) ? touch_count : max_touch_num;

	if (touch_count == 0) {
		finger_presence = 0;
		pr_notice("%s : touch up: max_num: %d\n", __func__, max_touch_num);
		max_touch_num = 0;
#ifdef ZTE_REPORT_KEY_PRESSURE
		if (report_key_flag) {
			pr_info("%s: report KEY_PRESSURE up\n", __func__);
...
#ifdef ZTE_REPORT_KEY_PRESSURE
	set_bit(KEY_PRESSURE, rmi4_data->input_dev->keybit);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_PRESSURE);
#endif
}
...
static int syna_i2c_reg_read(struct tpd_classdev_t *cdev, char addr, u8 *data, int len)
{
	...
	pr_info("syna_i2c_reg_read addr is 0x%x.", addr);
	return synaptics_rmi4_reg_read(rmi4_data, addr, data, len);
}
static int syna_i2c_reg_write(struct tpd_classdev_t *cdev, char addr, u8 *data, int len)
{
	struct synaptics_rmi4_data *rmi4_data = (struct synaptics_rmi4_data  *)cdev->private;

	pr_info("syna_i2c_reg_wite addr is 0x%x.", addr);
	return  synaptics_rmi4_reg_write(rmi4_data, addr, data, len);
}
	...
	int *p_fw_ver)
{
	...
	/*if (!bootloader_mode) {
		...
		dev_info(ts->pdev->dev.parent,
			...
			__func__);
		ret = syna_get_customer_id(&ts->config_id);
		if (ret < 0) {
			dev_err(ts->pdev->dev.parent,
				"%s: failed to get ts config id\n", __func__);
			return;
	...
	case 'X':
		...
		break;
	case 'b':
		snprintf(p_chip_type, CHIP_INFO_LEN, "S3706(0x%x)", ts->config_id.chip_type);
		break;
	default:
	...
	case 'I':
		...
		break;
	case 'Q':
		snprintf(p_sensor, CHIP_INFO_LEN, "GUOXIAN(0x%x)", ts->config_id.sensor);
		touch_module = GUOXIAN;
		break;
	case 'R':
	...
	*p_fw_ver = ts->config_id.fw_ver;
	...
	if (touch_module < SYN_MOUDLE_NUM_MAX) {
		if (touch_module == SAMSUNG)
			syna_file_name = FW_IMAGE_NAME_SAMSUNG;
		else if (touch_module == GVO)
			syna_file_name = FW_IMAGE_NAME_GVO;
...

{
	...
	len += snprintf(buffer + len, sizeof(buffer) - len, "FW Revision : %02x%02x\n",
	...
	if (ready_fw_ver < 0) {
		...
		len += snprintf(buffer + len, sizeof(buffer) - len, "ready fw version : %02x%02x\n",
			(ready_fw_ver & 0x0000ff00) >> 8, ready_fw_ver & 0x000000ff);
	}

	if (ready_fw_ver == fw_version)
		fw_flag = 0;
...
static int lockdown_info_show(struct seq_file *m, void *v)
{
	int ret = 0;
	int i = 0;
	...
	ret = syna_get_customer_id(&syn_ts->config_id);
	if (ret < 0) {
	...
	for (i = 0; i < USER_BUF_SIZE; i++) {
		if (customer_ID[i * 2 + 16] == 0)
			seq_printf(m, "%d", customer_ID[(i * 2 + 1) + 16]);
		else if (customer_ID[i * 2 + 16] == 1)
			seq_printf(m, "-%d", customer_ID[(i * 2 + 1) + 16]);
		else {
			pr_err("%s: invalid customer_ID valud\n", __func__);
			return -EINVAL;
		}
		if (i != (USER_BUF_SIZE - 1))
			seq_printf(m, ",");
	}
...
{
	return single_open(file, lockdown_info_show, NULL);
}

static ssize_t lockdown_info_write(struct file *file,
	const char __user *buffer, size_t count, loff_t *pos)
{
	int retval;
	char *user_buffer;
	...
	int i = 0;
	int token_to_int = 0;
	int user_value_tmp[USER_BUF_SIZE + 1] = {0};

	pr_info("%s: write proc\n", __func__);

	user_buffer = kmalloc(PAGE_SIZE, GFP_KERNEL);
	if (!user_buffer) {
		pr_err("%s: Failed to alloc mem for user_buffer\n", __func__);
		return -ENOMEM;
	}

	pr_info("%s: count=%ld\n", __func__, count);
	retval = copy_from_user(user_buffer, buffer, count);
	if (retval || count > (5 * USER_BUF_SIZE)) {
		pr_err("%s: invalid user_buffer\n", __func__);
		retval = -EINVAL;
		goto exit;
	}

	user_buffer[count - 1] = '\0';
	pr_info("%s: user_buffer: %s\n", __func__, user_buffer);

	while (((token = strsep(&user_buffer, delim)) != NULL) &&
		(i < (USER_BUF_SIZE + 1))) {
		if (kstrtoint(token, 10, &token_to_int)) {
			pr_err("%s: fail to convert str to int\n", __func__);
			retval = -EINVAL;
			goto exit;
		}

		if (token_to_int < -128 || token_to_int > 255) {
			pr_err("%s: invalid token_to_int :%d\n", __func__, token_to_int);
			retval = -EINVAL;
			goto exit;
		}
		user_value_tmp[i++] = token_to_int;

		pr_info("%s: %d,", __func__, user_value_tmp[i - 1]);
	}

	if (i > USER_BUF_SIZE) {
		pr_err("%s: invalid data length, i=%d\n", __func__, i);
		retval = -EINVAL;
		goto exit;
	}

	for (i = 0; i < USER_BUF_SIZE; i++) {
		if (user_value_tmp[i] >= 0) {
			user_value[i * 2] = 0;
			user_value[i * 2 + 1] = (unsigned char)user_value_tmp[i];
		} else {
			user_value[i * 2] = 1;
			user_value[i * 2 + 1] = (unsigned char)(0 - user_value_tmp[i]);
		}
	}

	retval = zte_write_lockdown_info();
	if (retval) {
		retval = -EINVAL;
		goto exit;
	}
...
static const struct file_operations  proc_lockdown_info = {
	.open = lockdown_info_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = lockdown_info_write,
...
static void synaptics_rmi4_probe_delay(struct work_struct *work)
{
	int retval;
	unsigned char attr_count;
	struct synaptics_rmi4_data *rmi4_data;
...
/*#ifdef CONFIG_FB
	...
	refresh = proc_create("lockdown_info", 0644, dir, &proc_lockdown_info);
	if (refresh == NULL)
		pr_notice("proc_create lockdown_info failed!\n");
	else
		pr_notice("proc_create lockdown_info success!\n");
...
static int synaptics_rmi4_probe(struct platform_device *pdev_tmp)
{
		pdev = pdev_tmp;
		if (pdev == NULL) {
		...
		syna_rmi4_probe_wq = create_singlethread_workqueue("syna_rmi4_probe_wq");
		INIT_WORK(&syna_rmi4_probe_work, synaptics_rmi4_probe_delay);
		queue_work(syna_rmi4_probe_wq, &syna_rmi4_probe_work);
...
{
	int retval;
	unsigned char smart_cover_reg = 0x1E;
	unsigned char smart_cover_enable = 0x02;
	unsigned char smart_cover_disable = 0x00;
	...
	if (enable) {
		retval = synaptics_rmi4_reg_write(rmi4_data,
			smart_cover_reg,
			&smart_cover_enable,
			sizeof(smart_cover_enable));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to enable smart_cover, retval = %d\n",
				__func__, retval);
			return;
		}
		...
		retval = synaptics_rmi4_reg_write(rmi4_data,
			smart_cover_reg,
			&smart_cover_disable,
			sizeof(smart_cover_disable));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to disable detect smart cover, retval = %d\n",
				__func__, retval);
			return;
		}
		...
		unsigned long event, void *data)
{
	...
	if (dev == NULL) {
		pr_err("%s: dev is null, do nothing!\n", __func__);
		return 0;
	}
...
int __init synaptics_rmi4_init(void)
{
	int retval;
...
#ifndef CONFIG_TOUCHSCREEN_SYNAPTICS_TCM
