...
#define CUSTOMER_ID_SAMSUNG	"A9PRO4801"
#define CUSTOMER_ID_GVO		"A9PRO5601"
...
unsigned char *syna_file_name;
static unsigned char fwname[128];
...
unsigned char config_data_tmp[512] = {0};
extern unsigned char user_value[USER_BUF_SIZE * 2];
...
int syna_get_customer_id(struct rmi_config_id *config_id)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_CUSTOMER_ID
	if (strnstr(customer_ID, CUSTOMER_ID_SAMSUNG, sizeof(CUSTOMER_ID_SAMSUNG)) != NULL) {
		pr_info("%s: S3706, SAMSUNG\n", __func__);
		config_id->chip_type = 0x62; /* S3706 */
		config_id->sensor = 0x48; /* SAMSUNG */
	} else if (strnstr(customer_ID, CUSTOMER_ID_GVO, sizeof(CUSTOMER_ID_GVO)) != NULL) {
		pr_info("%s: S3706, GVO\n", __func__);
		config_id->chip_type = 0x62; /* S3706 */
		config_id->sensor = 0x56; /* GVO */
	}
#else
...
static enum flash_area fwu_go_nogo(void)
{
	...
	for (ii = 0; ii < config_id_size; ii++) {
		pr_notice("%s: syna  fwu->img.ui_config.data[%d] = 0x%x, fwu->config_id[%d] = 0x%x\n",
...
static int fwu_write_pm_configuration(void)
{
	...
	pr_info("%s: write config data\n", __func__);
	for (i = 0; i < 512; i++) {
		if ((i % 16) == 0)
			pr_info("\n");
		pr_info("0x%x", fwu->config_data[i]);
	}
...
static int zte_write_config(void)
{
	int retval;
	unsigned short config_area;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
int zte_write_lockdown_info(void)
{
	int retval = 0;
	int i = 0;
	int j = 0;
	...
	pr_info("%s: start writing lockdown info for lcd\n", __func__);
	...
	for (i = 0; i < FIXED_LENGTH; i++)
		config_data_tmp[i] = fwu->read_config_buf[i];

	/* copy the new data */
	for (j = 0; j < (USER_BUF_SIZE * 2); j++)
		config_data_tmp[FIXED_LENGTH + j] = user_value[j];

	fwu->config_data = config_data_tmp;

	retval = zte_write_config();
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
int syna_get_fw_ver(struct i2c_client *client, unsigned char *pfwfilename)
{
	int fwver = -1;
	unsigned char *charcmp = "";
	const struct firmware *fw_entry = NULL;
