...
#define MSM_BOARD_ID_T3        7 /*pine*/
#define MSM_BOARD_ID_POAD_T3  13 /*poad*/

#define MAX_CURRENT_UV  1800000
#define MIN_CURRENT_UV  1800000
...
/*@zte board id, begin*/
static struct smtc_reg_data sx9325_i2c_reg_temp[] = {
	/*Interrupt and config*/
	{
		.reg = SX9325_IRQ_ENABLE_REG,
		.val = 0x60,
...
#if 0
static struct smtc_reg_data sx9325_i2c_reg_sleep[] = {
	/*Interrupt and config*/
	{
		.reg = SX9325_IRQ_ENABLE_REG,
		.val = 0x00,
	...
	if (chip) {
		while (i < num) {
			if (chip->board_id == MSM_BOARD_ID_T3) {
				write_register(chip, sx9325_i2c_reg_temp[i].reg, sx9325_i2c_reg_temp[i].val);
				read_register(chip, sx9325_i2c_reg_temp[i].reg, &tempval);
				i++;
			} /*else if (chip->board_id >= MSM_BOARD_ID_POAD_T3) {
				write_register(chip, sx9325_i2c_reg_sleep[i].reg, sx9325_i2c_reg_sleep[i].val);
				read_register(chip, sx9325_i2c_reg_sleep[i].reg, &tempval);
				i++;
			} */else {
...
#ifdef BOARD_USES_SAR_CHIP
static int sx9325_config_regulator(psx93XX_t chip, bool on)
{
	int rc = 0;

	pr_info("%s:vdd on %d\n", __func__, on);
	if (on) {
		chip->sx9325_vdd = devm_regulator_get(chip->pdev, "vdd");
		if (IS_ERR(chip->sx9325_vdd)) {
			pr_err("unable to get sx9325 vdd\n");
			rc = PTR_ERR(chip->sx9325_vdd);
			chip->sx9325_vdd = NULL;
			return rc;
		}
		if (regulator_count_voltages(chip->sx9325_vdd) > 0) {
			rc = regulator_set_voltage(
					chip->sx9325_vdd,
					MIN_CURRENT_UV,
					MAX_CURRENT_UV);
			if (rc) {
				pr_err("Regulator vddio Set voltage failed rc=%d\n", rc);
				goto deinit_vregs;
			}
		}
		return rc;
...
deinit_vregs:
	if (regulator_count_voltages(chip->sx9325_vdd) > 0)
		regulator_set_voltage(chip->sx9325_vdd, 0, MAX_CURRENT_UV);
...
static int sx9325_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (chip->board_id <= MSM_BOARD_ID_POAD_T3) {
		err = sx9325_config_regulator(chip, true);
	}

	g_sx9325_chip = chip;
	fb_register_client(&sx9325_fb_notifier);
