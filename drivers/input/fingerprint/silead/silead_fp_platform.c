		...
		if (fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START]) {
			...
			if (fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START] != KEY_RESERVED) {
				if (pkey->flag == NAV_KEY_FLAG_CLICK) {
					input_report_key(fp_dev->input, fp_dev->keymap_cust.k
							[pkey->value - NAV_KEY_START], NAV_KEY_FLAG_DOWN);
					input_sync(fp_dev->input);
					input_report_key(fp_dev->input, fp_dev->keymap_cust.k
							[pkey->value - NAV_KEY_START], NAV_KEY_FLAG_UP);
					input_sync(fp_dev->input);
				} else {
					input_report_key(fp_dev->input, fp_dev->keymap_cust.k
							[pkey->value - NAV_KEY_START], pkey->flag);
					input_sync(fp_dev->input);
				}
			} else {
...
#endif /* PROC_DIR */
		fp_dev->proc_entry = proc_create(PROC_NODE, 0644, fp_dev->proc_root, &silfp_proc_fops);
		if (fp_dev->proc_entry == NULL) {
