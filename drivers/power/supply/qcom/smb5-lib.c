...
int smblib_dp_dm(struct smb_charger *chg, int val)
{
	...
	case POWER_SUPPLY_DP_DM_DP_PULSE:
		...
		if (rc < 0) {
			...
			if (pval.intval < MICRO_5V && chg->pulse_cnt > 10) {
			...
			} else if (chg->pulse_cnt < MAX_PULSE) {
				...
				smblib_dbg(chg, PR_MISC, "DP PULSE execeed max pulse, rerun apsd\n");
...
static int get_rp_based_dcp_current(struct smb_charger *chg, int typec_mode)
{
	...
	case POWER_SUPPLY_TYPEC_SOURCE_HIGH:
		/* zmodify, always use 2A,because 3A cause some charger collapse
...
static void smblib_raise_qc3_vbus_work(struct work_struct *work)
{
	...
	if (usb_present) {
		...
		vote_force_active(chg->fcc_main_votable, true, force_fcc);
...
#ifndef CONFIG_VENDOR_CPU_RATE_CTRL
	if (super_charge_mode) {
		if (usb_present.intval) {
			smblib_dbg(chg, PR_MISC, "In super_charge_mode, set to USB_SCR_OFF\n");
			chg->system_temp_level_offset = USB_SCR_OFF;
		} else if (dc_present.intval) {
			smblib_dbg(chg, PR_MISC, "In super_charge_mode, set to DC_SCR_OFF\n");
			chg->system_temp_level_offset = DC_SCR_OFF;
		}
	}
#endif
