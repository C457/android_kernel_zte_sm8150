...
static int old_capacity = 0, capacity = 0, battery_charging_enabled = 1;
...
struct p922x_dev {
	...
	struct delayed_work  chip_reset_work;
...
int wireless_charging_signal_good;
...
static irqreturn_t p922x_power_good_handler(int irq, void *dev_id)
{
	...
	if (!chip->power_good) {
		cancel_delayed_work_sync(&chip->chip_reset_work);
		schedule_delayed_work(&chip->chip_reset_work, msecs_to_jiffies(10000));
		cancel_delayed_work_sync(&chip->device_key_auth_work);
...
{
	struct p922x_dev *chip = container_of(work, struct p922x_dev,
						chip_reset_work.work);

	wake_lock(&chip->p922x_wake_lock);
	idtp922x_set_enable(chip, false);
	msleep(500);
	if (!chip->en_pin_enabled) {
		p922x_dbg(chip, PR_DEBUG, "en_pin_enabled is false, skip\n");
		return;
	}
...
static void e_trans_work(struct work_struct *work)
{
	...
	if (chip->power_good && chip->idt_rx_iout < WLS_RX_IOUT_MIN
		...
		if (capacity == 100 || !battery_charging_enabled) {
			p922x_dbg(chip, PR_DEBUG, "rx iout too small, but soc is 100, skip reset\n");
		} else {
		...
		const union power_supply_propval *val)
{
	...
	if (chip) {
		...
		case POWER_SUPPLY_PROP_PIN_ENABLED:
			...
			chip->en_pin_enabled = val->intval;
			break;
		case POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION:
...
static void p922x_chg_detect_work(struct work_struct *work)
{
	struct p922x_dev *chip = container_of(work, struct p922x_dev,
	...
	pr_info("p922x_chg_detect_work enter..\n");
	psy = power_supply_get_by_name("usb");
	if (!psy) {
		p922x_dbg(chip, PR_DEBUG, "Couldn't get usb psy, return\n");
		return;
	}
	power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE, &val);
	if (val.intval) {
		p922x_dbg(chip, PR_DEBUG, "usb_online:%d, skip\n", val.intval);
		return;
	}
				...
				unsigned long event, void *data)
{
	...
	battery_charging_enabled = val.intval;

	if (capacity == 100 && old_capacity != 100) {
		/* increase RX voltage to 6.5V */
		pr_info("capacity = 100, increase RX voltage ..\n");
		chip->bus.write(chip, REG_VOUT_SET, V_TERM/100 - 35);
	}
...
static int p922x_register_power_notifier(struct p922x_dev *chip)
{
	int rc = 0;
...
static int zte_misc_get_wireless_charging_signal(char *val, const void *arg)
{
	return  snprintf(val, PAGE_SIZE, "%d", wireless_charging_signal_good);
}

struct zte_misc_ops wireless_charging_signal_good_node = {
	.node_name = "wireless_charging_signal_good",
	.set = NULL,
	.get = zte_misc_get_wireless_charging_signal,
	.free = NULL,
	.arg = NULL,
};
...
static int p922x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	zte_misc_register_callback(&wireless_charging_signal_good_node, NULL);
	p922x_register_power_notifier(chip);

	schedule_delayed_work(&chip->chg_detect_work, msecs_to_jiffies(3000));
