...
#define POWER_SUPPLY_PROP_CHARGE_VOLTAGE_MAX_ZTE POWER_SUPPLY_PROP_PD_VOLTAGE_MAX
#endif

#ifndef POWER_SUPPLY_PROP_CHARGE_CURRENT_MAX_ZTE
#define POWER_SUPPLY_PROP_CHARGE_CURRENT_MAX_ZTE POWER_SUPPLY_PROP_PD_CURRENT_MAX
#endif

#ifndef POWER_SUPPLY_PROP_RECHARGE_SOC_ZTE
#define POWER_SUPPLY_PROP_RECHARGE_SOC_ZTE POWER_SUPPLY_PROP_SOC_REPORTING_READY
#endif
...
static int charge_arbitrate_parse_dt(struct charge_arbitrate_info *cas_info)
{
	...
	OF_READ_PROPERTY_STRINGS(cas_info->config_prop.bms_phy_name,
...
static int charge_arbitrate_debug_battery_info(struct charge_arbitrate_info *cas_info)
{
	...
	rc = power_supply_get_property(cas_info->bms_psy,
				POWER_SUPPLY_PROP_CAPACITY_RAW, &capacity_raw);
	if (rc < 0) {
...
static int charge_arbitrate_r1_p1_status_chargefull(struct charge_arbitrate_info *cas_info)
{
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		if ((cas_info->run_prop.run_rech_soc != 0)/*check soc_now less than recharge soc*/
			...
			&& ((voltage_max.intval - voltage_ocv.intval) < cas_info->run_prop.run_rech_voltage)) {
			cas_info->force_charger_full = false;
			charge_arbitrate_set_input_suspend(cas_info, false);
...
static void charge_arbitrate_probe_work(struct work_struct *work)
{
	...
	cas_info->bms_psy = power_supply_get_by_name(cas_info->config_prop.bms_phy_name);
	if (!cas_info->bms_psy) {
...
battery_psy_failed:
#ifdef KERNEL_ABOVE_4_1_0
	power_supply_put(cas_info->bms_psy);
#endif
	cas_info->bms_psy =  NULL;
bms_psy_failed:
